...
class PachongSpider(RedisSpider):
    ...
    def make_request_from_data(self, data):
        ...
        page_max = 79
        form_data = {'nian': '2020', 'jidu': '一季报', 'zoom': '1', 'trade': '', 'sort': '', 'where': '', 'page': ''}
        for page in range(1, page_max + 1):
            data['page'] = str(page)

        return scrapy.FormRequest(url=url, callback=self.parse, formdata=form_data, cookies=cookies)
    ...
    #     for page in range(1, page_max+1):
    #         data['page'] = str(page)
    #         yield scrapy.FormRequest(url=url, callback=self.parse, formdata=form_data, cookies=cookies)
